...
def cmd(json_file, rubric_file, csv, gradescope):
    ...
    if not os.path.exists(rubric_file):
        print("Missing rubric file: {}".format(rubric_file), file=sys.stderr)
        sys.exit(1)
    ...
    if "test-points" not in config:
        print("Error: Rubric file {} does not have a [test-points] section.".format(args.rubric_file), file=sys.stderr)
        sys.exit(1)
